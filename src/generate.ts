...
export async function generateFilesFromInitState(
  ...
  try {
    ...
    if (!stateExists) {
      ...
      if (options.verbose) {
        console.log(chalk.yellow(`⚠️  state.ts already exists at ${stateFilePath}`));
        console.log(chalk.blue(`💡 Consider manually adding: ${sliceName}: ${interfaceName}`));
      }
    }
    ...
    if (options.verbose && !options.dryRun) {
      ...
      console.log(chalk.cyan(`   • slice.ts - Redux slice with ${keys.length} actions`));
      console.log(chalk.cyan(`   • reducers.ts - Reducer export`));
      console.log(chalk.cyan(`   • types.ts - TypeScript interface`));
      console.log(chalk.cyan(`   • state.ts - ${stateExists ? 'already exists' : 'created'}`));
    }
